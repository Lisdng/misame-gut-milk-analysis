...
timepoint_order <- c("incl", "tri3", "pn12", "pn56")
...
mother_genus_plot <- mother_genus %>%
  ...
  ggplot(aes(x = timepoint, y = Abundance, fill = genus_grouped)) +
...
infant_genus_plot <- infant_genus %>%
  mutate(timepoint = factor(timepoint, levels = sample_order_genus),
         genus_grouped = factor(genus_grouped, levels = genus_order)) %>%
  ggplot(aes(x = timepoint, y = Abundance, fill = genus_grouped)) +
  geom_bar(stat = 'identity', position = 'fill') +
  scale_fill_manual(breaks=genus_name, values = genus_color) + 
...
pn12e_meta_species <- pn12e_meta %>%
  tax_glom(taxrank = "species") %>%
  transform_sample_counts(function(x) x / sum(x)) %>%
  psmelt() %>%
  filter(Abundance > 0)

pn12e_species_counts <- pn12e_meta_species %>%
  dplyr::group_by(idbs) %>%
  dplyr::summarise(num_species = n_distinct(OTU)) %>%
  dplyr::mutate(timepoint = "pn12e",
                idbs = str_c(idbs, "e")) 
  

pn12m_meta_species <- pn12m_meta %>%
  tax_glom(taxrank = "species") %>%
  transform_sample_counts(function(x) x / sum(x)) %>%
  psmelt() %>%
  filter(Abundance > 0)

pn12m_species_counts <- pn12m_meta_species %>%
  dplyr::group_by(idbs) %>%
  dplyr::summarise(num_species = n_distinct(OTU)) %>%
  dplyr::mutate(timepoint = "pn12m",
                idbs = str_c(idbs, "m")) 
  

pn12h_meta_species <- pn12h_meta %>%
  tax_glom(taxrank = "species") %>%
  transform_sample_counts(function(x) x / sum(x)) %>%
  psmelt() %>%
  filter(Abundance > 0)

pn12h_species_counts <- pn12h_meta_species %>%
  dplyr::group_by(idbs) %>%
  dplyr::summarise(num_species = n_distinct(OTU)) %>%
  dplyr::mutate(timepoint = "pn12h",
                idbs = str_c(idbs, "h")) 

shared_pn12epn12m_species <- pn12e_meta_species %>%
  semi_join(pn12m_meta_species, by = "OTU")

shared_pn12epn12m_species_counts <- shared_pn12epn12m_species %>%
  dplyr::group_by(idbs) %>%
  dplyr::summarise(num_species = n_distinct(OTU)) %>%
  dplyr::mutate(timepoint = "pn12em",
                idbs = str_c(idbs, "m")) 

pn12_species_counts <- rbind(pn12h_species_counts, pn12e_species_counts, pn12m_species_counts) %>%
  mutate(timepoint = factor(timepoint, levels = c("pn12h", "pn12e", "pn12m")))



pn12SpeciesNumber <- ggplot(pn12_species_counts, aes(x = num_species, fill = factor(timepoint))) +
  geom_density(alpha = 0.5) +
  labs(x = "Number of Species", y = "Density", fill = "Group", title = "1-2-month gut & milk") +
  scale_fill_manual(values = c("#2986cc", "#f1c232", "#6aa84f" ), labels = c("Human milk", "Infant gut", "Maternal gut")) +
  theme_minimal() +
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank()) +
  geom_vline(xintercept = median(pn12h_species_counts$num_species), color = "#2986cc", linetype = "dashed") +
  geom_vline(xintercept = median(pn12e_species_counts$num_species), color = "#f1c232", linetype = "dashed") +
  geom_vline(xintercept = median(pn12m_species_counts$num_species), color = "#6aa84f", linetype = "dashed")  
pn12SpeciesNumber


#svglite::svglite(
#  filename="./gut_milk/pn12SpeciesNumber100.svg",
#  width=6, height=3
#  )
#pn12SpeciesNumber
#dev.off()
...
## Fig1C bottom pn34 & pn56

```{r message=FALSE}

pn56e_meta_species <- pn56e_meta %>%
  tax_glom(taxrank = "species") %>%
  transform_sample_counts(function(x) x / sum(x)) %>%
  psmelt() %>%
  filter(Abundance > 0)

pn56e_species_counts <- pn56e_meta_species %>%
  dplyr::group_by(idbs) %>%
  dplyr::summarise(num_species = n_distinct(OTU)) %>%
  dplyr::mutate(timepoint = "pn56e",
                idbs = str_c(idbs, "e")) 
  

pn56m_meta_species <- pn56m_meta %>%
  tax_glom(taxrank = "species") %>%
  transform_sample_counts(function(x) x / sum(x)) %>%
  psmelt() %>%
  filter(Abundance > 0)

pn56m_species_counts <- pn56m_meta_species %>%
  dplyr::group_by(idbs) %>%
  dplyr::summarise(num_species = n_distinct(OTU)) %>%
  dplyr::mutate(timepoint = "pn56m",
                idbs = str_c(idbs, "m")) 
  

pn34h_meta_species <- pn34h_meta %>%
  tax_glom(taxrank = "species") %>%
  transform_sample_counts(function(x) x / sum(x)) %>%
  psmelt() %>%
  filter(Abundance > 0)

pn34h_species_counts <- pn34h_meta_species %>%
  dplyr::group_by(idbs) %>%
  dplyr::summarise(num_species = n_distinct(OTU)) %>%
  dplyr::mutate(timepoint = "pn34h",
                idbs = str_c(idbs, "h")) 


shared_pn56epn56m_species <- pn56e_meta_species %>%
  semi_join(pn56m_meta_species, by = "OTU")

shared_pn56epn56m_species_counts <- shared_pn56epn56m_species %>%
  dplyr::group_by(idbs) %>%
  dplyr::summarise(num_species = n_distinct(OTU)) %>%
  dplyr::mutate(timepoint = "pn56em",
                idbs = str_c(idbs, "m")) 

pn34pn56_species_counts <- rbind(pn34h_species_counts, pn56e_species_counts, pn56m_species_counts) %>%
  mutate(timepoint = factor(timepoint, levels = c("pn34h", "pn56e", "pn56m")))


pn34pn56SpeciesNumber <- ggplot(pn34pn56_species_counts, aes(x = num_species, fill = factor(timepoint))) +
  geom_density(alpha = 0.5) +
  labs(x = "Number of Species", y = "Density", fill = "Group", title = "5-6-month gut & 3-4-month milk") +
  scale_fill_manual(values = c("#2986cc", "#f1c232", "#6aa84f"), labels = c("Human milk", "Infant gut", "Maternal gut")) +
  theme_minimal() +
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank()) +
  geom_vline(xintercept = median(pn34h_species_counts$num_species), color = "#2986cc", linetype = "dashed") +
  geom_vline(xintercept = median(pn56e_species_counts$num_species), color = "#f1c232", linetype = "dashed") +
  geom_vline(xintercept = median(pn56m_species_counts$num_species), color = "#6aa84f", linetype = "dashed") 
pn34pn56SpeciesNumber


#svglite::svglite(
#  filename="./gut_milk/pn34pn56SpeciesNumber100.svg",
#  width=6, height=3
#  )
#pn34pn56SpeciesNumber
#dev.off()
...
## Fig1D shared all genus

```{r message=FALSE}

pn12e_meta_genus <- pn12e_meta %>%
  tax_glom(taxrank = "genus") %>%
  transform_sample_counts(function(x) x / sum(x)) %>%
  psmelt() %>%
  filter(Abundance > 0) %>%
...
incl_meta_genus <- incl_meta %>%
  tax_glom(taxrank = "genus") %>%
  transform_sample_counts(function(x) x / sum(x)) %>%
  psmelt() %>%
  filter(Abundance > 0)  %>%
  dplyr::mutate(timepoint = "incl") %>%
  select(genus, Abundance, idbs, timepoint)

tri3_meta_genus <- tri3_meta %>%
  tax_glom(taxrank = "genus") %>%
  transform_sample_counts(function(x) x / sum(x)) %>%
  psmelt() %>%
  filter(Abundance > 0)  %>%
  dplyr::mutate(timepoint = "tri3") %>%
  select(genus, Abundance, idbs, timepoint)

pn12m_meta_genus <- pn12m_meta %>%
  tax_glom(taxrank = "genus") %>%
  transform_sample_counts(function(x) x / sum(x)) %>%
  psmelt() %>%
  filter(Abundance > 0)  %>%
...
X1421h_meta_genus <- X1421h_meta %>%
  tax_glom(taxrank = "genus") %>%
  transform_sample_counts(function(x) x / sum(x)) %>%
  psmelt() %>%
  filter(Abundance > 0) %>%
  dplyr::mutate(timepoint = "1421h")  %>%
  select(genus, Abundance, idbs, timepoint) %>%
  mutate(genus = str_replace(genus, "_\\d{6}$", ""))

pn12h_meta_genus <- pn12h_meta %>%
  tax_glom(taxrank = "genus") %>%
  transform_sample_counts(function(x) x / sum(x)) %>%
  psmelt() %>%
  filter(Abundance > 0) %>%
...
pn12_genus <- rbind(pn12e_meta_genus, pn12m_meta_genus, pn12h_meta_genus) %>%
  mutate(timepoint = factor(timepoint, levels = c("pn12h", "pn12e", "pn12m")))

all_genus <- rbind(pn12e_meta_genus, pn56e_meta_genus, incl_meta_genus, tri3_meta_genus, pn12m_meta_genus, pn56m_meta_genus, X1421h_meta_genus, pn12h_meta_genus, pn34h_meta_genus)

# Example dataset 'all_genus'
# Assuming you have a dataset with columns: genus, Abundance, idbs, timepoint

# Step 1: Reshape the data to create a presence-absence matrix
genus_timepoint_matrix <- all_genus %>%
  distinct(genus, timepoint) %>%
  mutate(presence = 1) %>%
  spread(key = timepoint, value = presence, fill = 0)

# Step 2: Define the custom order of timepoints and color groups
custom_timepoint_order <- c("incl", "tri3", "pn12m", "pn56m", "1421h", "pn12h", "pn34h", "pn12e", "pn56e")
existing_timepoints <- custom_timepoint_order[custom_timepoint_order %in% colnames(genus_timepoint_matrix)]

# Define color groups
color_group_1 <- c("incl", "tri3", "pn12m", "pn56m")
color_group_2 <- c("1421h", "pn12h", "pn34h")
color_group_3 <- c("pn12e", "pn56e")

 
# Assign colors to timepoint sets
set_colors <- c(rep("#6aa84f", length(color_group_1)),  # Color for Group 1
                rep("#2986cc", length(color_group_2)),   # Color for Group 2
                rep("#f1c232", length(color_group_3))) # Color for Group 3

# Step 3: Create an UpSet plot with custom set colors
upset <- upset(genus_timepoint_matrix, 
      sets = existing_timepoints,  # Use the existing timepoints in custom order
      keep.order = TRUE, 
      order.by = "freq", 
      main.bar.color = "#104E8E", 
      sets.bar.color = set_colors,
  nintersects = NA)  

#svglite::svglite(
#  filename="./gut_milk/sept10-allGenus_allIntersets.svg",
#  width=10, height=5
#  )
#upset
#dev.off()
...
# Step 1: Extract unique genera for each source group
mother_genera <- unique(c(
  incl_meta_genus$genus,
  tri3_meta_genus$genus,
  pn12m_meta_genus$genus,
  pn56m_meta_genus$genus
))

infant_genera <- unique(c(
  pn12e_meta_genus$genus,
  pn56e_meta_genus$genus
))

milk_genera <- unique(c(
  X1421h_meta_genus$genus,
  pn12h_meta_genus$genus,
  pn34h_meta_genus$genus
))

# Step 2: Create a named list of sets
venn_list <- list(
  Mother = mother_genera,
  Infant = infant_genera,
  Milk   = milk_genera
)

# Step 3: Create the Venn plot (no filename, store it as object)
venn.plot <- venn.diagram(
  x = venn_list,
  filename = NULL,  # keep it NULL to store the object
  col = "black",
  fill = c("#2986cc", "coral1", "slateblue2"),
  alpha = 0.7,
  cex = 1.5,
  cat.cex = 1.3,
  cat.pos = 0,
...
# Step 4: Save to SVG
#svglite::svglite(filename = "./gut_milk/sept10-allGenus_VENN.svg", width = 5, height #= 5)
#grid::grid.draw(venn.plot)  # this actually draws the plot
#dev.off()
...
## 🤍 for reviewer - relative abundance
...
# Run Fisher’s exact test for each timepoint
...
doc_table <- doc_lines %>%
  ...
  dplyr::mutate(
    FullName = str_replace_all(
      FullName,
      c(
        "Entéroagrégatif" = "Enteroaggregative",
        "Entérotoxinogène" = "Enterotoxigenic",
...
infant_species_pn56 <- infant_species %>%
  ...
  select(idbs, species, Abundance) %>%
  pivot_wider(names_from = species, values_from = Abundance) %>%
  dplyr::select(idbs, where(~ mean(is.na(.)) <= 0.93))  # <- updated: keep only species with ≤ 50% missing
...
auto_improve_temporal <- function(
  ...
  repeat {
    ...
    if (inherits(fm, "try-error")) {
      if (verbose) cat("⚠️  Fit measures unavailable (non-converged?).\n")
      break
    }
    ...
    if (!any(is.na(fm)) && fm["cfi"] >= cfi_thr && fm["rmsea"] <= rmsea_thr && fm["srmr"] <= srmr_thr) {
      if (verbose) cat("✅ Model fit is acceptable. Stopping.\n")
      break
    }
    if (step >= max_steps) {
      if (verbose) cat("⛔ Reached max steps. Stopping.\n")
      break
    }
    ...
    if (inherits(mi, "try-error") || is.null(mi) || nrow(mi) == 0L) {
      if (verbose) cat("⚠️ No modification indices available. Stopping.\n")
      break
    }
    mi_reg <- subset(mi, op == "~" & !grepl("\\|", lhs) & !grepl("\\|", rhs) & mi >= min_mi)

    if (!nrow(mi_reg)) {
      if (verbose) cat("⚠️ No MI above threshold. Stopping.\n")
      break
    }
    ...
    if (!nrow(mi_allowed)) {
      if (verbose) cat("⚠️ All high-MI paths violate temporality or already exist. Stopping.\n")
      break
    }
    ...
    for (i in seq_len(nrow(mi_allowed))) {
      ...
      if (improves) {
        if (verbose) cat(sprintf("➕ Adding: %s  [MI=%.2f]  -> CFI %.3f→%.3f, RMSEA %.3f→%.3f, SRMR %.3f→%.3f\n",
    ...
    if (!added) {
      if (verbose) cat("⚠️ No candidate path both respected time and improved fit. Stopping.\n")
      break
    }
  }

  if (verbose) {
    cat("\n📝 Final model after", step, "steps:\n")
    cat(current, "\n")
  }
